...
class PhyloTree(DataStruct):
    ...
    def __getHexColor(self, trackn, val):
        ...
        color = color.lower()
        if not PhyloTree.recolor.findall(color): raise Exception(
            'Wrong color format (HTML valid color name or color code (#ff00ff))')
        rng = maxval - minval
        val -= minval
        val = rng - val
        val = float(val) / rng
    ...
    def _refresh(self):
        ...
        for trackn, data in list(self.tracks.items()):
            for tipname, title in list(data[2].items()):
                self._update('addTreeTrackFeature', trackn, tipname, data[1],
                             title, False)

        for trackn, data in list(self.gradientTracks.items()):
            for tipname, [title, value] in list(data[4].items()):
                self._update('addTreeTrackFeature', trackn, tipname,
                             self.__getHexColor(trackn, value), title, value)
    ...
    def _refreshBars(self):
        for nbar, (description, color, minval, maxval, bars) in list(
                self.trackBars.items()):
            for tipname, value in list(bars.items()):
                ...
                self._update('addTreeTrackBar', value, val, tipname, color,
                             nbar, self.nbars)
...
.. code:: python
        ...
        if not 0 < trackn <= self.ntracks: raise Exception(
            "Track number does not exist")
        if trackn in self.gradientTracks:
            if title == None: title = str(value)
            if type(value) == bool: raise Exception(
                "You must specify a value for a gradient track")
            self.gradientTracks[trackn][4][tipname] = [title, value]
            self._update('addTreeTrackFeature', trackn, tipname,
                         self.__getHexColor(trackn, value), title, value)
        elif trackn in self.tracks:
            if value != False: raise Exception(
                "This track does not accept values")
            self.tracks[trackn][2][tipname] = title
            self._update('addTreeTrackFeature', trackn, tipname,
                         self.tracks[trackn][1], title, value)
        elif trackn in self.mcolorTracks:
            desc, colordict, feats = self.mcolorTracks[trackn]
            if value not in colordict: raise Exception(
                "Value not found in color dictionary")
            feats[tipname] = value
            self._update('addTreeTrackFeature', trackn, tipname,
                         colordict[value], value, value)
    ...
    def delTrackFeature(self, trackn, tipname):
        ...
        if tipname in self.tracks[trackn][2]: del self.tracks[trackn][2][
            tipname]
...
.. code:: python
        ...
        if not 0 < nbar <= self.nbars: raise Exception(
            "Bar number does not exist")
