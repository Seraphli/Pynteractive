...
class Graph(VisNetwork):
    ...
    def _refresh(self):
        ...
        for i in list(self.vertices.values()):
            ...
            self._update("addNode", i["_id"], i["_label"], i["_title"],
                         i["_group"], i["_shape"], i["_color"], i["_radius"],
                         i["_image"])
        for i, j in list(self.edges.items()):
            self._update("addEdge", i, j["_n1"], j["_n2"], j["_label"],
                         j["_title"], j["_width"], j["_style"])
...
.. csv-table:: Network matrix with distances (matrix=True,distances=True) 
   ...
   """
        ...
        if not matrix:
            ...
            if cols != rows:
                raise Exception("Rows andColumns do not match")
            for i in rows:
