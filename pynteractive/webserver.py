...
import re
from pynteractive.util import get_path
...
class ThreadingServer(ThreadingMixIn, HTTPServer):
    ...
    def force_stop():
        ...
        urllib.request.urlopen(
            "http://0.0.0.0:{0}/".format(pyn_globals.PORT)).read()
...
class FileMgr:
    def __init__(self, path=None, tarString=None):
        ...
        if tarString:
            self.contents = self.iniTar(
                io.StringIO(base64.b64decode(tarString)))
            self.METHOD = "tar"
        else:
            self.iniPath('.')
            self.METHOD = "path"
    ...
    def iniTar(self, tarStr):
        ...
        for tarinfo in tar:
            if tarinfo.isreg():
                ...
                pass  # print ("a directory.")
            else:
                pass  # print ("something else.")
...
class JSCom(WebSocket):
    def __init__(self, *args, **kwargs):
        ...
        funcs = [i for i in dir(self) if
                 inspect.ismethod(getattr(self, i)) and getattr(self,
                                                                i).__doc__ != "FWCLASS"]
        self.dicFuncs = {}
        self.dicArgs = {}
        self.dataId = None
    ...
    def handleMessage(self):
        ...
        try:
            ...
            traceback.print_exc()
            print("Error processing", "({0})".format(len(orig)), str(orig)[:30],
                  "...", str(orig)[-30:])
...
class SimpleWS(SimpleHTTPRequestHandler):
    FILE_MGR = FileMgr(
        path='webfiles/') if not pyn_globals.WEBFILES else FileMgr(
        tarString=pyn_globals.WEBFILES)

    def do_GET(self):
        if self.path.startswith("/?"):
            resource = "webfiles/index.html"
        else:
            resource = '/'.join(
                ["webfiles"] + self.path.split("?")[0].split("/"))
        ...
        if resource in SimpleWS.FILE_MGR:
            ...
            self.end_headers()
            print(resource)
            self.wfile.write(SimpleWS.FILE_MGR[resource].encode())
        else:
            self.send_response(404)
            self.send_header("Content-Type", mimetypes.types_map[".html"])
            self.end_headers()
            self.wfile.write(
                "<h1>File not found (404)</h1><br>We are so sorry :(<br><br><br>That's no so bad, we will die someday anyway<br>Have a good day :D".encode())
