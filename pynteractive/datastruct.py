...
class DataStruct:
    ...
    def __init__(self, name=None):
        ...
        if ID in DataStruct._OBJECTS:
            raise Exception("Object {0} already exists!")
    ...
    def readCsv(csvfile):
        ...
        if csv:
            delimiter = Counter(
                [i for i in csv[0] if i in "\t ;,|"]).most_common(1)
            if delimiter:
    ...
    def view(self):
        ...
        url = "http://0.0.0.0:{0}/?dataid={1}&vtype={2}&wsport={3}".format(
            pyn_globals.PORT, self._ID, self.__class__.__name__,
            pyn_globals.WEBSOCKETPORT)
        print(url)
