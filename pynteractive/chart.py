...
class Chart(DataStruct):
    TYPES = {'bar': 'Bar', 'scatter': 'Scatter', 'line': 'Line',
             'stack': 'Stack'}
    ...
    def _addValue(self, series, x, y, **kwargs):
        ...
        if 'size' in kwargs:
            if type(kwargs['size']) == str: kwargs['size'] = int(
                kwargs['size']) if kwargs['size'].isdigit() else float(
                kwargs['size'].replace(',', '.'))
        if type(x) == str: x = int(x) if x.isdigit() else float(
            x.replace(',', '.'))
        if type(y) == str: y = int(y) if y.isdigit() else float(
            y.replace(',', '.'))
...
.. code:: python
        ...
        assert (not data and not csv and x) or (not x and not csv and data) or (
                not data and not x and csv)
        assert len(x) == len(y)
        ...
        if sizes:
            ...
            for i in data:
                if len(i) < 2 or len(i) > 4:
                    raise Exception(
                        'You need at least 2 values in the data vector '
                        'and not more than 4')
                if len(i) == 3:
                    sh = 'circle'
                    sz = 0.5
                    if i[2] in Chart.SHAPES:
                        sh = i[2]
                    else:
                        sz = i[2]
    ...
    def addValue(self, idSeries, x, y, **kwargs):
        ...
        assert not set(kwargs) - {"size", "shape"}
        self._update("addSeriesData", idSeries,
                     self._addValue(idSeries, x, y, **kwargs))
