...
class Map(Network):
    ...
    try:
        CACHE = pickle.load(
            open(os.path.join(os.path.expanduser("~"), ".pynteractive"), 'rb'))
    except:
        CACHE = {}
    ...
    def _refresh(self):
        ...
        for i in list(self.vertices.values()):
            self._update("addNode", i["_id"], i["_label"], '', '', '',
                         i["_color"], i["_radius"], '', i["_lng"], i["_lat"])
        for i, j in list(self.edges.items()):
            self._update("addEdge", i, j["_n1"], j["_n2"], '', '', '', '', '',
                         j["_color"], j["_width"])

    def updateNode(self, node_id, radius=None, color=None, lng=None, lat=None,
                   place=None, country=None):
        """Updates a node in the Map"""
        Network.updateNode(self, node_id, radius=radius, color=color, lng=lng,
                           lat=lat, place=place, country=country)
        i = self.vertices[node_id]
        self._update("updateNode", i["_id"], i["_label"], '', '', '',
                     i["_color"], i["_radius"], '', i["_lng"], i["_lat"])
    ...
    def _addNode(self, node_id=None, radius=5, color='red', lng=None, lat=None,
                 place=None, country=None):
...
- place,country: If you don't use coordinates, you can specify a word the describes the place (eg, POSTCODE) and if you are worried that thare can be a comflict with another country you can use the country parameter that has to ISO 3166-1alpha2 compliant"""
        ...
        assert (lat == None and lng == None and place) or (
                    not place and lat != None and lng != None)
        ...
        try:
            if place:
                ...
                if lng == None:
                    print("Error getting coordinates for loading", place,
                          country)
                    Map.RUNNINGTHREADS.release()
                    return None, None

            _id, label = Network.addNode(self, node_id, node_id, radius=radius,
                                         color=color, lng=lng, lat=lat)
            self._update("addNode", node_id, node_id, '', '', '', color, radius,
                         '', lng, lat)
        except:
            pass
    ...
    def _getLocation(self, place, country=None):
        ...
        url = "http://open.mapquestapi.com/nominatim/v1/search.php?key=lYrP4vF3Uk5zgTiGGuEzQGwGIVDGuy24&format=json&limit=1&addressdetails=0&q={0}".format(
            place)
