...
class VisNetwork(Network):
    ...
    def addNode(self, node_id=None, label=None, title=None, group=None,
                shape='ellipse', color=None, radius=None, image=None):
...
- image: Image in case the shape is image or circularImage"""
        ...
        assert not shape or shape in ['ellipse', 'circle', 'box', 'database',
                                      'image', 'circularImage', 'label', 'dot',
                                      'star', 'triangle', 'triangleDown',
                                      'square']
        if shape in ['image', 'circularImage']: assert image != None

        node_id, label = Network.addNode(self, node_id, label, title=title,
                                         group=group, shape=shape, color=color,
                                         radius=radius, image=image)

        self._update("addNode", node_id, label, title, group, shape, color,
                     radius, image)
        return node_id, label

    def updateNode(self, node_id, label=None, title=None, group=None,
                   shape=None, color=None, radius=None, image=None):
        """Updates a node in the network"""
        Network.updateNode(self, node_id, label=label, title=title, group=group,
                           shape=shape, color=color, radius=radius, image=image)
        i = self.vertices[node_id]
        self._update("updateNode", i["_id"], i["_label"], i["_title"],
                     i["_group"], i["_shape"], i["_color"], i["_radius"],
                     i["_image"])
...
- length: length of the edge"""
        ...
        assert not style or style in ['line', 'arrow', 'arrow-center',
                                      'dash-line']
        ...
        _id, label = Network.addEdge(self, n1, n2, label, title=title,
                                     width=width, style=style, length=length)
